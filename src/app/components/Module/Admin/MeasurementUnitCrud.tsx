...
import { MeasurementUnitTable } from '../../Shared/Entity/Measure/MeasurementUnit/MeasurementUnitTable';
import { measurementUnitCrudState } from '../../../reducers/Module/Admin/MeasurementUnit/Crud';
import MeasurementUnitForm from '../../Shared/Entity/Measure/MeasurementUnit/MeasurementUnitForm';

/**
 * MeasurementUnitCrud
 *   MeasurementUnitCrud component
 * @param {any}  props Properties
 * @return {React.Component} MeasurementUnitCrud Component
 */
export const MeasurementUnitCrud = (props: any) => {
  const { fetchParams, pathParams, fixedFields, hiddenFields } = props;
  const dispatch = useDispatch();

  const measurementUnitCrudData = useSelector((state: RootState) => state.module.admin.measurementUnit.crud);

  // data
  const fetchResponse = measurementUnitCrudData.fetchResponse;
  const rows = fetchResponse?.items || [];
  const selected = measurementUnitCrudData.item;
  const action = measurementUnitCrudData.action;
  ...
  const formId = 'measurementUnit-crud-form';
  // actions
  const fetch = (fetchParams) => {
    dispatch({
      type: measurementUnitCrudState.FETCH,
  ...
  const save = (item) => {
    dispatch({
      type: isCreate ? measurementUnitCrudState.CREATE : measurementUnitCrudState.UPDATE,
  ...
  const remove = (item) => {
    dispatch({
      type: measurementUnitCrudState.DELETE,
      payload: { item, pathParams }
    })
  }
  ...
  const setAction = (action: ActionTypeEnum, item?: any) => {
    dispatch({
      type: measurementUnitCrudState.SET_ACTION,
